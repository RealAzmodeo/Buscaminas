...
export const BIOME_DEFINITIONS: Record<BiomeId, BiomeDefinition> = {
  [BiomeId.Default]: {
    ...
    icon: "⛰️",
    description:
      "El bioma por defecto, equilibrado. Un eco de lo familiar en la penumbra.",
  ...
  [BiomeId.BrokenBazaar]: {
    ...
    boardModifiers: (
      config: BoardParameters,
      _currentLevel: number,
      nodeRewardType?: MapRewardType,
    ): BoardParameters => {
      ...
      if (nodeRewardType === MapRewardType.ExtraGold || Math.random() < 0.3) {
        // 30% chance for biome itself to be richer
        if (
          config.objectRatioKey !== "richInGold" &&
          !config.objectRatioKey.includes("scarce")
        ) {
          // Avoid overriding strong themes
  ...
  [BiomeId.BloodForge]: {
    ...
    icon: "🔥",
    description:
      "Más Verdugos, mayor densidad de Ataques. Un calor antinatural emana de estas cámaras.",
    ...
    boardModifiers: (config: BoardParameters): BoardParameters => {
      ...
      if (
        config.objectRatioKey !== "manyAttacks" &&
        config.objectRatioKey !== "hostile"
      ) {
  ...
  [BiomeId.SilentLibrary]: {
    ...
    icon: "📜",
    description:
      "Menos enemigos, pero los que hay son más difíciles. Mayor probabilidad de Ecos relacionados con la información.",
...
export const REWARD_DESCRIPTIONS: Record<MapRewardType, string> = {
  ...
  [MapRewardType.ExtraGold]:
    "Camino del Avaro: Mayor generación de Oro en este tramo.",
  [MapRewardType.SoulFragments]:
    "Senda del Alma: Recompensa de Fragmentos de Alma al final del tramo.",
  [MapRewardType.WillLumens]:
    "Destello de Voluntad: Recompensa de Lúmenes de Voluntad al final del tramo.",
  [MapRewardType.FreeEcho]:
    "Santuario Olvidado: Se te ofrecerán 3 Ecos y podrás elegir uno sin coste.",
  [MapRewardType.HealingFountain]:
    "Santuario Olvidado: Recupera una cantidad significativa de HP.",
  [MapRewardType.EchoForge]:
    "Santuario Olvidado: Permite mejorar un Eco existente a su siguiente nivel.",
};
...
export const ENCOUNTER_DESCRIPTIONS: Record<MapEncounterType, string> = {
  ...
  [MapEncounterType.ArchetypeSpecific]:
    "Encuentro con arquetipo específico garantizado.",
